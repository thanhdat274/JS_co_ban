...
function initTable() {
    ...
    // duyệt danh sách và chọn thẻ input là checkbox trong các tr
    for (var i = 0; i < list_tr.length; i++) {
        var the_tr = list_tr[i];
        the_tr.style.display = "table-row"; //phải có dòng này để sau dùng làm căn cứ lọc

        var chk = the_tr.querySelector("input[type=checkbox]");

        //gắn thuộc tính onchange để gọi hàm số xử lý khi kích chuột thay đổi checkbox ở mỗi dòng
        chk.setAttribute("onchange", "changeCheckbox(this)");

        //chọn tiếp thẻ nhập số lượng để gắn thuộc tính onchange
...
function checkAll(objCHK) {
    ...
    var list_tr = document.querySelectorAll("table tbody tr");
    // duyệt mảng thẻ tr sau đó tìm thẻ checkbox trong thẻ tr
    for (var i = 0; i < list_tr.length; i++) {
        var the_tr = list_tr[i];
        var chk = the_tr.querySelector("input[type=checkbox]");
        chk.checked = kt_true;
